...
`POND_EVENTS_PATH` selects the combined JSONL event log used for terminal commands, tool calls, skills, thinking, and agent actions. If unset, Pond uses `$XDG_DATA_HOME/pond/events.jsonl`. `FISH_COMMAND_CAPTURE_PATH` and `POND_AGENT_EVENTS_PATH` remain compatibility fallbacks.
